...
public class BookDao {
	...
	public Book findByBid(String bid) throws SQLException {
		...
		//把Map中cid属性映射到Category中，既这个Category只有cid
		Category category=CommonUtils.toBean(map, Category.class);
		//两者建立关系
		book.setCategory(category);
		
		//把pid获取出来，创建一个Category parnet，把pid付给他，然后把parent付给category
		if(map.get("pid") != null) {
	...
	private PageBean<Book> findByCriteria(List<Expression> exprList,int pc) throws SQLException{
		...
		for(Expression expr : exprList) {
			 ...
			 * 4）如果条件不是is null，在追加问号，然后向params中添加一与问好对应的值
			 */
