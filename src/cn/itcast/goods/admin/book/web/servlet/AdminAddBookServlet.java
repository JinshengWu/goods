...
public class AdminAddBookServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		 ...
		 * 4.1首先把普通表单字段放到一个Map中，再把Map转换成Book和Category对象，在建立两者的关系
		 */
		 ...
		 */
		try {
			fileItem.write(destFile);//它会把临时文件重定向到指定的路径，在删除临时文件
		} catch (Exception e) {
	...
	private void error(String msg, HttpServletRequest request, HttpServletResponse response) 
		...
		request.setAttribute("parents", new CategoryService().findParents());//所有一级分类分类
